...
- 작품의 특징: 번역기능,위치에 따른 날씨정보 제공
 ...
 (5월 25일)
  기상청API를 활용해서 날씨정보를 얻어오는 것을 시도해봄.<br>
  공공데이터 포탈을 통해 강수 확률,강수 형태, 습도,하늘 상태, 기온의 데이터를 받아옴.<br>
  WeatherInterface를 통해 공공데이터 포탈에서 정보를 가져옴.<br>
  xml 파일 형식을 data class로 구현에서 데이터를 받아옴.<br>
  retrofit을 사용하기 위한 빌더를 생성하고 공공데이터 url을 등록.<br>
  getVilageFcst를 통해 단기예보를 조회해봄.<br>
